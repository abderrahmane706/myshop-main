import { getProduct, PRODUCTS, CATEGORIES } from '@/lib/data/products';

// ── Server-side metadata (runs on the server, no 'use client' needed) ──
...
import { Heart, Share2, Minus, Plus, Truck, Shield, RotateCcw, Star, ChevronRight, Check } from 'lucide-react';
      ...
      <div className="grid md:grid-cols-2 gap-8 lg:gap-14">
        ...
        <div>
          ...
          <div className="mt-5 flex items-baseline gap-3">
            <span className="text-4xl font-bold text-brand-primary">{formatMoney(product.price)}</span>
            {product.compare_at && (
              <>
                <span className="text-lg text-brand-text/40 line-through">{formatMoney(product.compare_at)}</span>
                <Badge variant="outline" className="border-brand-orange/30 text-brand-orange rounded-full">{t('product.save')} {formatMoney(savings)}</Badge>
              </>
            )}
          </div>
